...
bayesPredsF <- function(x, ...){
  UseMethod("bayesPredsF")
}

bayesPredsF.stanreg <- function(x, at, n_draws=2000, ci=.95, hdi_interval=TRUE, centrality='mean', digits=4, at_means=FALSE, data_slice="full", ...){

  predsErrorCheckF(model      = x,
                   at         = at,
...
bayesPredsF.data.frame <- function(x, model_data, model_formula, link_function, at, model_offset=NULL, n_draws=2000, ci=.95, hdi_interval=TRUE, centrality='mean', digits=4, at_means=FALSE, data_slice="full", ...){
